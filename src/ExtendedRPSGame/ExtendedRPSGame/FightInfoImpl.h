...
public:
	FightInfoImpl() {}
	FightInfoImpl(const Point& position, char player1Piece, char player2Piece, int winner) : mPlayersPieces{ player1Piece, player2Piece }, mWinner(winner) { mPos = std::make_unique<PointImpl>(position.getX(), position.getY()); }
